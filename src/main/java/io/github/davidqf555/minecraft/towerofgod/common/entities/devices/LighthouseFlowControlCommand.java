...
public class LighthouseFlowControlCommand extends DeviceCommand {
    ...
    public void tick() {
        ...
        targets.clear();
        targets.addAll(device.level.getLoadedEntitiesOfClass(LivingEntity.class, AxisAlignedBB.ofSize(range * 2, range * 2, range * 2).move(device.position()), EntityPredicates.NO_CREATIVE_OR_SPECTATOR.and(entity -> entity.distanceToSqr(device) <= range * range && !device.isAlliedTo(entity))));
