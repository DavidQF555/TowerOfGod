...
public class ObserverItem extends Item {
    ...
    public ActionResult<ItemStack> onItemRightClick(@Nonnull World worldIn, PlayerEntity playerIn, @Nonnull Hand handIn) {
        if (ShinsuTechnique.USE_OBSERVER.getBuilder().canCast(ShinsuTechnique.USE_OBSERVER, playerIn, 1, null, playerIn.getLookVec())) {
            IShinsuStats stats = IShinsuStats.get(playerIn);
            stats.cast(playerIn, ShinsuTechnique.USE_OBSERVER, 1, null, playerIn.getLookVec());
