...
public class SmeltDropsFilter implements DropsFilter {
    ...
    public ObjectArrayList<ItemStack> apply(LootContext context, ObjectArrayList<ItemStack> original) {
        ...
        for (ItemStack drop : original) {
            smelted.add(manager.getRecipeFor(RecipeType.SMELTING, new SimpleContainer(drop), world).map(AbstractCookingRecipe::getResultItem).filter(item -> !item.isEmpty()).map(item -> ItemHandlerHelper.copyStackWithSize(item, drop.getCount() * item.getCount())).orElse(drop));
        }
        return smelted;
    }
