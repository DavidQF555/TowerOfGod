...
public abstract class BasicShinsuUserEntity extends PathfinderMob implements IShinsuUser, IGeared<BasicShinsuUserEntity>, RangedAttackMob {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor worldIn, DifficultyInstance difficultyIn, MobSpawnType reason, @Nullable SpawnGroupData spawnDataIn, @Nullable CompoundTag dataTag) {
        ...
        if (group != null) {
            text = new TranslatableComponent(getType().getDescriptionId() + ".group_name", group.getName(), getShinsuStats().getLevel()).withStyle(group.getTextFormattingColor());
        } else {
            text = new TranslatableComponent(getType().getDescriptionId() + ".name", getShinsuStats().getLevel());
        }
        setCustomName(text);
