...
public class FloorTeleportationTerminalBlock extends Block {
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (player instanceof ServerPlayerEntity) {
            OpenFloorTeleportationTerminalMessage.INSTANCE.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) player), new OpenFloorTeleportationTerminalMessage(IShinsuStats.get(player).getLevel(), pos, state.get(FACING)));
        }
