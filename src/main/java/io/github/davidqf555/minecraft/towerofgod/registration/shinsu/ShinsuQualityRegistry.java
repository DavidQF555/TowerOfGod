...
public final class ShinsuQualityRegistry {
    ...
    public static final RegistryObject<ShinsuQuality> ICE = register("ice", () -> new ShinsuQuality(ParticleTypes.POOF, DamageSource.MAGIC, 1, 1, 0xFFa8fbff, new PotionQualityEffect(new EffectInstance(Effects.SLOWNESS, 60, 2, true, false, false)), new PlaceSphereQualityEffect<>(5, (world, pos) -> {
        ...
        if (state.getBlock().isAir(state, world, pos)) {
            ...
            if (snow.isValidPosition(world, pos)) {
                return snow;
            }
        } else if (state.getBlock() instanceof SnowBlock) {
            BlockState snow = state.with(SnowBlock.LAYERS, Math.min(state.get(SnowBlock.LAYERS) + 1, 8));
            if (snow.isValidPosition(world, pos)) {
                return snow;
            }
        }
    ...
    public static final RegistryObject<ShinsuQuality> STONE = register("stone", () -> new ShinsuQuality(new BlockParticleData(ParticleTypes.BLOCK, Blocks.STONE.getDefaultState()), DamageSource.FALLING_BLOCK, 0.8, 1.5, 0xFF999999, (user, clip) -> {
    }, new DestroyBlockQualityEffect((world, pos) -> {
        float hardness = world.getBlockState(pos).getBlockHardness(world, pos);
        return hardness >= 0 && hardness <= Blocks.STONE.getDefaultState().getBlockHardness(world, pos);
    ...
    public static final RegistryObject<ShinsuQuality> CRYSTAL = register("crystal", () -> new ShinsuQuality(new BlockParticleData(ParticleTypes.BLOCK, Blocks.GLASS.getDefaultState()), DamageSource.MAGIC, 0.9, 2, 0xFFf7f7f7, (user, clip) -> {
    }, (entity, clip) -> {
