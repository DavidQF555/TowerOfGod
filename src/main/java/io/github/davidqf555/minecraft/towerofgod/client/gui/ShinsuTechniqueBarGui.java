...
public class ShinsuTechniqueBarGui extends AbstractGui implements IRenderable {
    ...
    private static class Slot extends AbstractGui implements IRenderable {
        ...
        private static final TextureRenderData RENDER = new TextureRenderData(TEXTURE, 15, 16, 0, 0, 15, 15);
        private static final TextureRenderData COOLDOWN = new TextureRenderData(TEXTURE, 15, 16, 15, 0, 1, 1);
        ...
        public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
            ...
            ClientReference.render(RENDER, matrixStack, x, y, getBlitOffset(), WIDTH, HEIGHT, ColorHelper.PackedColor.blendColors(selected ? SELECTED_COLOR : UNSELECTED_COLOR, canCast ? CAN_CAST_COLOR : CANNOT_CAST_COLOR));
            ClientReference.render(technique.getIcon(), matrixStack, x + ICON_DIF_WIDTH, y + ICON_DIF_HEIGHT, getBlitOffset(), WIDTH - ICON_DIF_WIDTH * 2, HEIGHT - ICON_DIF_HEIGHT * 2, selected ? SELECTED_COLOR : UNSELECTED_COLOR);
            ...
            if (cooldown > 0) {
                ...
                ClientReference.render(COOLDOWN, matrixStack, x, y + HEIGHT - height, getBlitOffset(), WIDTH, height, ColorHelper.PackedColor.blendColors(COOLDOWN_COLOR, selected ? SELECTED_COLOR : UNSELECTED_COLOR));
            }
        }
